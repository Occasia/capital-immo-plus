# Meta Ad Copy & Video Scripts (Capital Immo Community)

This document contains the copy and scripts for the **4 ad creatives** to build the waitlist for the **Capital Immo Community**.

---

## Creative 1: Static Image / Graphic

### Visual Concept
A clean, modern, split-image graphic:
* **Left side**: A high-quality photo of Veronica Pillay smiling professionally.
* **Right side**: A clean mockup of a modern house in Montreal/Laval or a bold, solid brand-colored background with high-contrast text:
  * *"STOP RENTING. START OWNING."* or *"L'IMMOBILIER SIMPLIFIÉ."*
* **Overlay Badge**: *Capital Immo Community - Free Workshops*

---

### [EN] English Copy

* **Primary Text**:
  Renting forever isn't the plan. But with rising rates and a complex market, buying your first property in Montreal or Laval feels harder than ever. 
  
  You don't have to navigate it alone. 
  
  We are launching the **Capital Immo Community**—a space for local renters, first-time buyers, and future investors to get together, share real numbers, and learn from real case studies.
  
  We’re hosting a series of **Free In-Person Workshops** covering:
  ✅ How to prepare your finances and down payment for today's rates
  ✅ Crucial mistakes to avoid in your first purchase
  ✅ How to analyze duplexes/triplexes for rental income
  
  We are officially opening the waitlist. Seats will be limited for the first in-person session.
  
  👇 Click below to join the waitlist and secure your spot today.
* **Headline**: Join the Capital Immo Community Waitlist 🔑
* **Description**: Free real estate workshops in Montreal & Laval.
* **CTA Button**: Sign Up

---

### [FR] French Copy

* **Primary Text**:
  Payer un loyer toute sa vie n'est pas votre plan. Pourtant, avec la hausse des taux et la complexité du marché, acheter sa première propriété à Montréal ou à Laval semble plus difficile que jamais.
  
  Vous n'avez pas à le faire seul.
  
  Nous lançons la **Communauté Capital Immo** — un espace d'entraide pour les locataires, les premiers acheteurs et les futurs investisseurs locaux afin d'échanger, d'analyser des chiffres réels et d'apprendre d'études de cas concrètes.
  
  Nous organisons une série d'**Ateliers Présentiels Gratuits** sur :
  ✅ Comment préparer vos finances et votre mise de fonds face aux taux actuels
  ✅ Les erreurs cruciales à éviter lors de votre premier achat
  ✅ Comment analyser un duplex ou triplex pour générer des revenus locatifs
  
  Nous ouvrons officiellement la liste d'attente. Les places seront limitées pour la première session.
  
  👇 Cliquez ci-dessous pour rejoindre la liste d'attente dès aujourd'hui.
* **Headline**: Rejoignez la liste d'attente Capital Immo 🔑
* **Description**: Ateliers immobiliers gratuits à Montréal et Laval.
* **CTA Button**: S'inscrire (Sign Up)

---
---

## Creative 2: B-Roll Video with Text Overlay

### Video Concept
* **Length**: 15–20 seconds
* **Visuals**: A compilation of high-quality vertical B-roll:
  * 0:00 - 0:05: Scenic drone shot of residential streets in Montreal or Laval (duplexes/triplexes).
  * 0:05 - 0:10: Close-up of hands typing numbers on a laptop or sketching on a notebook.
  * 0:10 - 0:15: Veronica Pillay in a modern meeting room talking/collaborating.
  * 0:15 - 0:20: A clean graphic transition showing: *Capital Immo Community (Waitlist Open)*.
* **Music**: Smooth, modern, low-fi or high-energy tech-business beat.

---

### Text Overlay (On-Screen Text)
* *0:00 - 0:05*: "Rent is going up... but buying feels impossible? 📈" / *"Le loyer augmente... mais acheter semble impossible ? 📈"*
* *0:05 - 0:10*: "No more guessing games. Real numbers. Real strategies. 📊" / *"Fini les suppositions. Des vrais chiffres. Des vraies stratégies. 📊"*
* *0:10 - 0:15*: "I'm building a community of future buyers & investors. 🤝" / *"Je bâtis une communauté de futurs acheteurs et investisseurs. 🤝"*
* *0:15 - 0:20*: "Join the Capital Immo Community. Link below. 🔑" / *"Rejoignez la Communauté Capital Immo. Lien ci-dessous. 🔑"*

---

### [EN] English Copy

* **Primary Text**:
  Are you tired of paying rent but feel stuck on how to take the next step? 
  
  The Capital Immo Community is here to bridge the gap. We are hosting free, practical workshops in Montreal and Laval to help you buy your first home or investment property with confidence.
  
  No sales pitches. Just real numbers, local market data, and actionable advice.
  
  Join the waitlist now to get notified of our first event!
* **Headline**: Stop Renting, Start Owning 🏡
* **Description**: Join the community waitlist for free workshops.
* **CTA Button**: Learn More

---

### [FR] French Copy

* **Primary Text**:
  Fatigué de payer un loyer mais bloqué sur la façon de passer à l'étape suivante ?
  
  La Communauté Capital Immo est là pour vous guider. Nous organisons des ateliers pratiques gratuits à Montréal et Laval pour vous aider à acheter votre premier foyer ou immeuble de placement en toute confiance.
  
  Pas de pitch de vente. Juste des vrais chiffres, des données locales et des conseils concrets.
  
  Rejoignez la liste d'attente pour être informé de notre premier événement !
* **Headline**: Arrêtez de louer, devenez propriétaire 🏡
* **Description**: Rejoignez la liste d'attente pour nos ateliers gratuits.
* **CTA Button**: En savoir plus (Learn More)

---
---

## Creative 3: Facecam Video - Variation A (Focus: Pillar A - First-Time Buyers)

### Video Concept
* **Speaker**: Veronica Pillay (looking directly into the lens).
* **Setting**: A bright, professional, and welcoming space (office, clean home background, or in front of local properties).
* **Length**: ~45-60 seconds.
* **Style**: Relatable, authentic, direct. Captions/subtitles burned in on screen.

---

### Video Script (Bilingual Tracks)

#### [FR] French Script (Afro Business Club Structure)
> "Si tu es locataire ici à Montréal ou à Laval, et que tu rêves d'acheter ta première propriété, c'est pour toi. 
> 
> Je crois qu'il n'y a pas assez d'espace pour nous rencontrer, poser nos questions et parler d'immobilier de façon transparente, sans pression d'achat. Alors on l'a bâti. 
> 
> On est en train de réunir les futurs acheteurs les plus ambitieux de la région pour nos ateliers en personne. Pas de blabla de courtier ou de jargon financier. Juste des vrais chiffres, de la vraie préparation financière et du concret pour débloquer ton premier achat. 
> 
> Je suis Veronica Pillay, fondatrice de Capital Immo Plus. On bâtit la communauté de futurs propriétaires la plus solide en ville. 
> 
> Si tu veux être dans la salle pour notre prochain atelier gratuit... n'attends pas de voir passer la pub. Rejoins gratuitement notre Liste d'Invitation Privée ci-dessous. C'est là qu'on envoie les invitations et les places en premier. On bâtit ensemble."

#### [EN] English Script
> "If you are currently renting here in Montreal or Laval and dream of buying your first home, this is for you. 
> 
> I believe there aren't enough spaces for us to meet, ask questions, and talk about real estate transparently, without sales pressure. So, we built it. 
> 
> We are bringing together the most ambitious future home buyers in the area for our in-person workshops. No broker talk, no complex financial jargon. Just real numbers, real financial preparation, and concrete steps to unlock your first purchase. 
> 
> I'm Veronica Pillay, founder of Capital Immo Plus. We are building the strongest community of future homeowners in town. 
> 
> If you want to be in the room for our next free workshop... don't wait for another ad to pop up. Join our free Private Invite List below. That's where we send invites and seats first. We build together."

---

### Ad Metadata (Bilingual)
* **[FR] Headline**: Acheter ton premier foyer à Mtl/Laval? 🔑
* **[FR] Description**: Rejoins notre liste d'invitation privée.
* **[EN] Headline**: Buying your first home in Mtl/Laval? 🔑
* **[EN] Description**: Join our private invitation list.
* **CTA Button**: Sign Up / S'inscrire

---
---

## Creative 4: Facecam Video - Variation B (Focus: Pillar B - Aspiring Investors)

### Video Concept
* **Speaker**: Veronica Pillay (looking directly into the lens).
* **Setting**: Office or standing in front of a duplex/triplex property.
* **Length**: ~45-60 seconds.
* **Style**: Focused, analytical, encouraging. Captions/subtitles burned in.

---

### Video Script (Bilingual Tracks)

#### [FR] French Script (Afro Business Club Structure)
> "Si tu veux investir dans ton premier duplex ou triplex ici à Montréal ou à Laval, c'est pour toi. 
> 
> Je crois qu'il n'y a pas assez d'espace pour analyser des deals locaux, parler de cashflow et comprendre les lois du TAL de façon transparente et sans filtre. Alors on l'a bâti. 
> 
> On est en train de rassembler des futurs investisseurs immobiliers déterminés pour des ateliers pratiques. Pas de blabla théorique ou de formations bidon à des milliers de dollars. Juste de la vraie analyse de fiches MLS, des calculs de rentabilité réels et du partage d'expérience terrain. 
> 
> Je suis Veronica Pillay, fondatrice de Capital Immo Plus. On est en train de bâtir la communauté d'investisseurs immobiliers la plus active et solide en ville. 
> 
> Si tu veux être dans la salle pour notre prochain atelier pratique gratuit... n'attends pas de voir passer la pub. Rejoins gratuitement notre Liste d'Invitation Privée ci-dessous. C'est là qu'on envoie les invitations en premier. On bâtit ensemble."

#### [EN] English Script
> "If you want to invest in your first duplex or triplex here in Montreal or Laval, this is for you. 
> 
> I believe there aren't enough spaces for us to analyze local deals, discuss cashflow, and understand TAL housing laws transparently and without filters. So, we built it. 
> 
> We are bringing together motivated future real estate investors for hands-on workshops. No theoretical talk, no overpriced courses costing thousands. Just real MLS deal analysis, real cashflow calculations, and real-world field experience. 
> 
> I'm Veronica Pillay, founder of Capital Immo Plus. We are building the most active and solid real estate investor community in town. 
> 
> If you want to be in the room for our next free practical workshop... don't wait for another ad to pop up. Join our free Private Invite List below. That's where we send invitations first. We build together."

---

### Ad Metadata (Bilingual)
* **[FR] Headline**: Investir dans ton premier duplex/triplex? 📊
* **[FR] Description**: Rejoins notre liste d'invitation privée.
* **[EN] Headline**: Investing in your first duplex/triplex? 📊
* **[EN] Description**: Join our private invitation list.
* **CTA Button**: Learn More / En savoir plus
